"""The scaled-damage KB gated on the full specialist panel.

`posture_panel_gate.py`'s harness, arm flag swapped: our Ogerpon list piloted
by agent/main.py against the eight field entries (matched harvested
specialists where one exists, the fitness generalist elsewhere), two arms,
CABT_SCALED_DAMAGE=0 and 1, identical seed blocks, seats swapped every other
game. This is the POOLED no-regression half of the gate; the TARGETED cell —
500 games against the codex_alakazam specialist, where the hand-size scaler
should concentrate — runs through `specialist_gate.py` with the env set, and
the ship rule is pre-registered as: targeted cell improves AND the panel
shows no regression (weighted clean delta not below -1.5 SE, no per-cell
clean delta below -2 SE). The panel is not required to improve — six of its
eight entries carry no big-hand engine, so the pooled read is a guard rail,
not the instrument.
...
before the opponent pilot is built, so a generalist opponent never receives
the treatment.

    python scripts/scaled_damage_gate.py --games 500 --workers 8
"""
...
def _init(root: str, a_path: str, deck_a: str, arm: str,
    ...
    # The treatment: our module reads CABT_SCALED_DAMAGE at import.
    os.environ["CABT_SCALED_DAMAGE"] = arm
    ...
    mod = _load_main(a_path, "sdg_a")
    if mod.SCALED_DAMAGE_ENABLED != (arm == "1"):
        raise RuntimeError(f"arm {arm}: flag did not land in our module")
    if arm == "1" and not mod._scalers():
        raise RuntimeError("arm 1: attack_scalers.json resolved empty")
    _STATE["a"] = mod.agent
    _STATE["deck_a"] = load_deck(Path(deck_a))
    _STATE["deck_b"] = list(deck_b)

    # The opponent never gets the treatment, whichever arm this is.
    os.environ["CABT_SCALED_DAMAGE"] = "0"
    if pilot_spec["kind"] == "external":
        ...
        if pilot._jon.SCALED_DAMAGE_ENABLED:
            raise RuntimeError("generalist opponent imported with the KB on")
...
def summarize(cells: list[dict]) -> dict:
    ...
    # The pre-registered pooled criterion is NO REGRESSION, not improvement:
    # this panel is the guard rail, the targeted alakazam cell (run through
    # specialist_gate.py) is the instrument.
    w = out["weighted_clean"]
    out["pooled_no_regression"] = (w["delta"] > -1.5 * w["se_delta"]
...
def main() -> None:
    ...
    ap.add_argument("--seed", type=int, default=320000)
    ap.add_argument("--out", default="data/analysis/scaled_damage_gate.json")
    args = ap.parse_args()
